...
| 当前阶段 | 已接入传送带居中闭环、手动传送带控制、状态查询和三台步进电机运行时参数下发；机械臂、称重、电感结果命令字先保留。 |

## 本次创建或修改文件

| 文件 | 类型 | 修改原因 |
|---|---|---|
| `User/App/binary_protocol_service.h` | 修改 | 定义协议帧格式、命令字、错误码、负载结构、CRC/解帧/分发接口；新增 `STEPPER_PARAM_SET(0x42)` 和 25 字节步进参数负载。 |
| `User/App/binary_protocol_service.c` | 修改 | 实现 CRC16-CCITT-FALSE、组帧、解帧、负载解码、ACK/NACK 发送和命令分发；新增三台步进电机参数校验和分发。 |
...
| `tmp/binary_protocol_service_host_test.c` | 修改 | Windows 主机侧测试 CRC、解帧、CRC 错误、`PAUSE_CYCLE/RESUME_CYCLE`、`VISION_POS` 和 `STEPPER_PARAM_SET` 负载解码，不依赖 F4 硬件。 |
...
|---|---|---:|---|
| `HELLO/HEARTBEAT/START_CYCLE/PAUSE_CYCLE/RESUME_CYCLE/STOP_CYCLE/VISION_POS/VISION_LOST/BELT_STOP_CENTERED/BELT_MANUAL_CONTROL/STEPPER_PARAM_SET` 执行成功 | `ACK 0x80` | 7 | `acked_seq`、`acked_cmd`、`cycle_id` 匹配，且 `status=0` 才算本次命令成功。 |
