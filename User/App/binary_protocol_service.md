...
| `User/App/uart_command.h` | 修改 | 增加 `UartCommand_SendRaw()` 和 `UartCommand_GetMp157Huart()`，用于发送包含 `0x00` 的二进制 ACK/NACK 帧，并集中管理 MP157 主链路串口句柄。 |
| `User/App/uart_command.c` | 修改 | 让二进制原始帧发送和 `my_printf()` 共用发送互斥锁；当前二进制主链路已改回 USART1，主链路文本输出默认静默。 |
...
| `User/App/weight_service.c` | 修改 | 在 MP157 主链路唯一命令消费者中，优先识别 `A5 5A` 二进制帧，再处理机械臂帧和 ASCII 文本命令；新增二进制称重标定上下文和文本/二进制共用标定核心。 |
...
| 资源 | 用途 | 参数 |
|---|---|---|
| USART1 | MP157/串口助手到 F4 的主通信入口 | 115200 8N1，PA9(TX)、PA10(RX)，DMA2 Stream2 Channel4 + IDLE 接收 |
...
| 2 | `actuator` | `u8` | `0/1/2` | `0=传送带`，`1=摄像头左右轴`，`2=摄像头上下轴`。 |
| 3 | `direction` | `u8` | `0/1` | 传送带 `0=后退/1=前进`，左右轴 `0=左移/1=右移`，上下轴 `0=下降/1=上升`，再由运行时方向映射到实际 CW/CCW。 |
...
| 2 | `actuator` | `u8` | `0/1` | 只允许传送带和摄像头左右轴；上下轴必须用固定步数位置模式。 |
| 3 | `direction` | `u8` | `0/1` | 传送带 `0=后退/1=前进`，左右轴 `0=左移/1=右移`。 |
...
> USART1 主链路上的 `[OK]`、`[ERROR]`、`[INFO]` 文本在正式 MP157 主链路默认关闭，不能作为正确或错误判断依据；当前业务代码里的 `my_printf(&huart1, ...)` 会被静默，若需要现场日志应迁移到非主链路串口。
...
| F4 -> MP157 ACK/NACK/STATUS/FAULT | `BinaryProtocolService_SendFrame()` 调用 `UartCommand_SendRaw(UartCommand_GetMp157Huart(), ...)` 原样发送。 | MP157 按相同帧格式解析；若串口助手查看，必须用 HEX 显示。 |
...
| MP157 收不到 ACK | 检查 `UartCommand_SendRaw()` 是否被编译进工程，USART1 TX/PA9 是否接到 MP157 RX，USART1 RX/PA10 是否接到 MP157 TX。 |
...
| 步进参数保存后 F4 没变化 | 确认 Qt 弹窗点的是 `保存并下发`，界面显示的下发 ID 是否为现场期望值，例如 `左右=3，上下=2`；F4 是否返回 `ACK acked_cmd=0x42`；USART1 作为 MP157 主链路时文本日志默认静默，不能再依赖 `Runtime config applied` 文本确认；代码已写不等于 F4 已烧录生效，必须重新编译并下载 F407 固件。 |
| 手动传送带或左右轴按一次后没有持续运动 | 确认 Qt 发的是 `ACTUATOR_VEL_MOVE 0x52`，不是旧的 `ACTUATOR_POS_MOVE`；确认 ACK `status=0`，再查传送带/左右轴电机供电、地址和 UART。 |
| 左右轴停止键 ACK 后仍继续运动 | 优先确认 F4 已烧录包含 `camera_motor_service.c` 的 `stop_epoch` 修复版本；旧版本虽然把 STOP 插到队首，但队列里残留的旧 `JOG` 可能在 STOP 后继续执行，表现为“停止没反应”。当前 USART1 是 MP157 主链路，文本日志默认静默，应以二进制 ACK/EVENT 和电机实际停止作为主要判断。 |
...
| 2026-07-02 | 历史阶段 USART1 主链路返回收敛为二进制：正确返回 `ACK/STATUS_REPORT`，错误返回 `NACK/FAULT_REPORT`；文本 `[OK]/[ERROR]` 默认静默，不再作为 MP157 判断依据。 |
...
| 2026-07-16 | MP157-F4 主链路从 USART1(PA9/PA10) 迁移到 USART2(PA2/PA3)；ACK/NACK/STATUS/FAULT 均从 USART2 返回，USART1 改为现场文本调试输出口。 |
| 2026-07-17 | 按现场要求把 MP157-F4 主链路改回 USART1(PA9/PA10)；`BinaryProtocolService_SendFrame()` 改为通过 `UartCommand_GetMp157Huart()` 获取主链路句柄，避免回包继续误发到 USART2。 |
