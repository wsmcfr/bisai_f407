...
| 摄像头前进/后退电机 | 现场默认 `0x03` | `USART6` | `PC6(TX) / PC7(RX)` | 负责摄像头前后方向微调；可由 `STEPPER_PARAM_SET` 更新 F4 运行时地址。 |
| 摄像头上下电机 | 现场默认 `0x02` | `USART6` | `PC6(TX) / PC7(RX)` | 负责摄像头上下方向调节或焦距/高度标定；可由 `STEPPER_PARAM_SET` 更新 F4 运行时地址。 |

结论：

| 问题 | 答案 |
|---|---|
| 摄像头前进/后退步进电机 ID | `0x03` |
| 摄像头上下移动步进电机 ID | `0x02` |
...
| `User/App/camera_motor_service.c` | 摄像头运动电机服务绑定 `huart6`，默认维护现场前后轴地址 `0x03` 和上下轴地址 `0x02`；新增运行时配置 FIFO 队列命令，STOP 队首优先。 | 可以通过 `CAM...` 文本命令调试摄像头前后轴和上下轴，也可以由 MP157 下发运行时参数。 |
...
| `CAMSTOP` | 同时停止摄像头前后轴和上下轴。 | `USART6 PC6/PC7 addr=0x03/0x02` |
| `CAMFWD FORWARD [rpm]` | 摄像头前进/后退轴按工程约定前进方向点动。 | `USART6 PC6/PC7 addr=0x03` |
| `CAMFWD BACKWARD [rpm]` | 摄像头前进/后退轴按工程约定后退方向点动。 | `USART6 PC6/PC7 addr=0x03` |
| `CAMZ UP [rpm]` | 摄像头上下轴按工程约定上升方向点动。 | `USART6 PC6/PC7 addr=0x02` |
| `CAMZ DOWN [rpm]` | 摄像头上下轴按工程约定下降方向点动。 | `USART6 PC6/PC7 addr=0x02` |
...
| 停止方式 | 发送 `CAMSTOP`，F4 会分别给前后轴地址 `0x03` 和上下轴地址 `0x02` 发送立即停止命令。 |
...
| 2 | 把摄像头前进/后退电机地址设置为 `0x03`。 | 后续 `CAMFWD ...` 只会控制这个轴。 |
| 3 | 断开前进/后退电机，只给摄像头上下电机上电。 | 单独设置第二个电机地址。 |
| 4 | 把摄像头上下电机地址设置为 `0x02`。 | 后续 `CAMZ ...` 只会控制这个轴。 |
...
| 查询摄像头电机 | USART1 串口助手 | `CAMINFO` | 输出 `forward_addr=3`、`z_addr=2`、`min_step`、`speed`、`dir` 和 `USART6=PC6/PC7`。 | 如果提示未知命令，检查 `weight_service.c` 是否已接入 `CameraMotorService_HandleCommand()`；如果仍是 `forward_addr=2,z_addr=3`，说明 F4 还没有重新编译下载新固件或 MP157 参数没有重新下发。 |
...
| 测试摄像头上升轴 | USART1 串口助手 | `CAMZ UP 30` | 只有摄像头上下轴动作。 | 如果前后轴动作，检查现场地址是否设置为前后 `0x03`、上下 `0x02`。 |
...
| USART6 到摄像头前后轴 | 串口助手发送 `CAMFWD FORWARD 30`。 | 观察摄像头前后轴动作，再发 `CAMSTOP`。 | 只有地址 `0x03` 电机动作表示 ID 正确。 |
...
| 2026-07-02 | 新增摄像头运动电机服务；早期规划前进/后退电机 ID 为 `0x02`、上下电机 ID 为 `0x03`，2026-07-04 已按现场实物调整。 |
...
| 2026-07-04 | 现场电机地址改为摄像头前进/后退轴 `0x03`、摄像头上下轴 `0x02`；参数保存 ACK 后还要通过 `CAMINFO` 或 `Runtime config applied` 确认 F4 运行内存已应用。 |
