...
| 摄像头左右电机 | 现场默认 `0x03` | `USART6` | `PC6(TX) / PC7(RX)` | 负责摄像头左右方向微调；可由 `STEPPER_PARAM_SET` 更新 F4 运行时地址。 |
...
| `CAMSTOP` | 同时停止摄像头左右轴和上下轴。 | `USART6 PC6/PC7 addr=0x03/0x02` |
| `CAMLAT RIGHT [rpm]` | 摄像头左右轴按工程约定右移方向点动。 | `USART6 PC6/PC7 addr=0x03` |
| `CAMLAT LEFT [rpm]` | 摄像头左右轴按工程约定左移方向点动。 | `USART6 PC6/PC7 addr=0x03` |
...
| 停止方式 | 发送 `CAMSTOP`，F4 会分别给左右轴地址 `0x03` 和上下轴地址 `0x02` 发送立即停止命令。 |
| 方向说明 | `LEFT/RIGHT/UP/DOWN` 是工程约定方向；如果实物方向反了，可在 MP157 参数页把对应轴方向设为反向并下发。`CAMFWD FORWARD/BACKWARD` 只是旧调试别名。 |

### 4.3 二进制步进参数命令

| CMD | 作用 | 关键约束 |
|---:|---|---|
| `0x42 STEPPER_PARAM_SET` | 一次下发传送带、摄像头左右、摄像头上下三台电机的地址、最小步长、常规速度和方向。 | `cycle_id=0`、`motor_count=3`、`flags=0`；地址 `1~247`，步长 `1~10000`，速度 `0~5000 rpm`，方向只能 `1/-1`。 |
...
| 步骤 | 操作 | 目的 |
|---:|---|---|
| 1 | 只给摄像头左右电机上电，断开摄像头上下电机。 | 避免两个默认地址电机同时响应地址设置命令。 |
| 2 | 把摄像头左右电机地址设置为 `0x03`。 | 后续 `CAMLAT ...` 只会控制这个轴。 |
| 3 | 断开左右电机，只给摄像头上下电机上电。 | 单独设置第二个电机地址。 |
...
| 2026-07-02 | 新增摄像头运动电机服务；早期规划横向电机 ID 为 `0x02`、上下电机 ID 为 `0x03`，2026-07-04 已按现场实物调整。 |
| 2026-07-02 | 新增 `CAMINFO/CAMSTOP/CAMLAT/CAMZ（CAMFWD 兼容）` 调试命令，用于现场确认两个摄像头电机是否能独立动作。 |
