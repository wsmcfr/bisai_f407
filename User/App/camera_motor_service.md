...
| 当前阶段 | `CAMLAT LEFT/RIGHT` 负责左右轴文本调试，`CAMZ UP/DOWN` 负责上下轴文本调试；MP157 通过 `STEPPER_PARAM_SET 0x42` 下发两台摄像头电机运行时地址、最小步长、常规速度和方向；`ACTUATOR_POS_MOVE/ACTUATOR_STOP/ACTUATOR_VEL_MOVE/ACTUATOR_HOME` 分别支持位置微调、停止、持续速度模式和当前位置设零；二进制位置运动收到张大头 Emm42 `[addr FD 9F 6B]` 时上报 `EVENT_REPORT event=0x14 status=0`，未收到主动回包但估算运动时间到期时上报 `event=0x14 status=5(estimated-done)`，只有 UART 读取错误或命令发送失败才上报 `event=0x15`。 |
...
| `User/App/binary_protocol_service.c/.h` | 把 `role_id=2`、`actuator=1` 的注释和分发目标改为摄像头左右轴，编号不变；新增 `BINARY_PROTOCOL_ACTUATOR_MOVE_STATUS_ESTIMATED_DONE=5`，用于标记没有收到 Emm42 主动回包时的估算完成。 | MP157/F4 二进制协议保持兼容，只改变业务语义；MP157 可从 `detail_i32` 低 16 位区分真实回包到位和估算完成。 |
| `User/Driver/emm42_motor.c/.h` | 同步通用驱动注释中的摄像头左右轴地址 `0x03`、上下轴地址 `0x02`。 | 避免现场按旧地址文档接线或排查。 |
| `User/App/camera_motor_service.c/.h` | 新增 `CameraMotorService_RequestLateralPositionWithReport()` 和 `CameraMotorService_RequestZPositionWithReport()`，并在任务循环中轮询 Emm42 到位回包；估算运动时间到期时发送 `ACTUATOR_MOVE_DONE status=5` 兜底。 | MP157 不再把 ACK 当成 Z 轴下降/回升完成，必须等 `ACTUATOR_MOVE_DONE` 后才复查 ROI 或启动机械臂；现场日志可区分 `status=0` 和 `estimated-done`。 |
| `User/Driver/emm42_motor.c/.h` | 新增到位回包非阻塞解析器，识别 `[addr FD 9F 6B]`；新增 `EMM42_MOTOR_STATUS_ESTIMATED_DONE`。 | 张大头 Response 配置为 `Reached` 或 `Both` 时可得到真实到位回包；未配置或 RX 无主动回包时，F4 仍按估算完成推进流程。 |
...
| `CameraMotorService_RequestLateralPositionWithReport(right_flag, speed_rpm, pulse_count, cycle_id, related_seq, actuator, direction)` | 请求左右轴位置运动，并把完成或真实故障事件回报 MP157，完成来源由 `status=0/5` 区分。 | `related_seq` 必须来自原始 `ACTUATOR_POS_MOVE`，MP157 用它匹配事件。 |
...
| `CameraMotorService_RequestZPositionWithReport(up_flag, speed_rpm, pulse_count, cycle_id, related_seq, actuator, direction)` | 请求上下轴位置运动，并把完成或真实故障事件回报 MP157，完成来源由 `status=0/5` 区分。 | 首页自动流程必须等该事件；本地定时器只作为超时保护。 |
...
| Z 轴到位事件 | MP157 首页自动流程或二进制串口工具 | 发送 `ACTUATOR_POS_MOVE actuator=2 direction=0/1 steps>0`。 | F4 先返回 ACK；收到 `[addr FD 9F 6B]` 时返回 `EVENT_REPORT event=0x14 status=0 related_seq=<本次SEQ>`；没有主动回包但估算运动时间到期时返回 `event=0x14 status=5`；UART 读取错误或发送失败才返回 `event=0x15`。 | 若总是 `status=5`，检查张大头 Response 是否为 `Reached/Both`、USART6 RX 是否接好、上下轴地址是否为 `0x02`、电源和共地；流程不会再因为缺少主动回包永久卡死。 |
