...
| 摄像头左右电机 | 现场默认 `0x03` | `USART6` | `PC6(TX) / PC7(RX)` | `User/App/camera_motor_service.c` |
...
| 摄像头左右步进电机 ID | 现场默认 `0x03`，可由 MP157 `STEPPER_PARAM_SET` 更新 F4 运行时参数 |
...
| 2 | `User/App/camera_motor_service.c` | `CameraMotorService_HandleCommand()` | 解析 `CAMINFO/CAMSTOP/CAMLAT/CAMZ（CAMFWD 兼容）`。 |
| 3 | `User/App/camera_motor_service.c` | `CameraMotorService_Task()` | 独占 `USART6`，串行控制两个不同地址的 Emm42 电机。 |
| 4 | `User/Driver/emm42_motor.c` | `EMM42_MotorSetVelocity()` / `EMM42_MotorStopNow()` | 地址 `0x03` 控制左右轴，地址 `0x02` 控制上下轴。 |
...
| `CAMSTOP` | 同时停止左右轴和上下轴。 | `USART6 PC6/PC7 addr=0x03/0x02` |
| `CAMLAT RIGHT [rpm]` | 摄像头左右轴右移点动。 | `USART6 PC6/PC7 addr=0x03` |
| `CAMLAT LEFT [rpm]` | 摄像头左右轴左移点动。 | `USART6 PC6/PC7 addr=0x03` |
...
字段约束：地址 `1~247`，最小步长 `1~10000 step`，`normal_speed_rpm` 和 `scan_speed_rpm` 都为 `0~5000 rpm`，方向只能 `1/-1`。其中传送带 `scan_speed_rpm` 用于零件未入画时的 SCAN 上料扫描，传送带 `normal_speed_rpm` 用于零件入画后的 TRACK 对中和短步微调；摄像头两轴使用 `normal_speed_rpm` 作为省略 rpm 时的默认动作速度，`scan_speed_rpm` 固定填 `0`。
...
| 运行时参数 | MP157 下发 `STEPPER_PARAM_SET` 后，任务先进入 STOP，再更新当前地址、对中/短步速度、上料扫描速度和方向映射。 |
...
| 2026-07-02 | 早期规划摄像头横向电机使用 `USART6 PC6/PC7 addr=0x02`，后续已按现场实物调整。 |
