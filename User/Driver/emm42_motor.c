 ...
 *
 * 通信链路：
 * - 本驱动是通用 Emm42 TTL 帧发送层，不直接绑定某一个业务电机；
 * - 传送带服务当前绑定 `UART4`，PC10(TX) 接传送带 Emm42 RX，PC11(RX) 接传送带 Emm42 TX，地址为 `0x01`；
 * - 摄像头运动服务当前绑定 `USART6`，PC6(TX) 接两个摄像头 Emm42 RX，PC7(RX) 接两个摄像头 Emm42 TX；
 * - 摄像头前进/后退电机地址为 `0x02`，摄像头上下电机地址为 `0x03`，两者共用 `USART6` 时必须靠地址区分；
 ...
 *
 * 当前实现直接使用阻塞发送：
 * 1. Emm42 命令帧很短，阻塞时间可控；
 * 2. 不依赖额外 TX DMA 配置，适合当前工程现状；
 * 3. 同一条串口总线如果挂多个电机，必须由上层服务保证发送互斥，避免两条命令交叉。
 */
