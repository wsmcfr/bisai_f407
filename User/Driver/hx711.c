...
/**
 * @brief HX711 双线时序协议与用户命令关系速查。
 *
 * 本文件是 HX711 的底层 GPIO 时序驱动，不直接接收 USART1 文本命令。
 * 用户可从 USART1 发送的称重命令由 `weight_service.c` 解析：
 * | 用户命令 | 最终调用 | 作用 | 是否会让硬件动作 |
 * | --- | --- | --- | --- |
 * | `GET` | `HX711_ReadRaw()` + `HX711_ConvertToGrams()` | 读取当前重量或原始净计数 | 否，只读取 ADC |
 * | `TARE` | `HX711_Tare()` | 多次空载采样并刷新 `offset` 零点 | 否，但会改变后续重量零点 |
 * | `CAL <克重>` | `HX711_CalibrateByKnownWeight()` | 用已知砝码计算 `scale_counts_per_g` | 否，但会改变后续重量比例 |
 *
 * HX711 时序要点：
 * 1. DOUT 被 HX711 拉低表示一次 24bit 转换结果已经准备好；
 * 2. MCU 通过 SCK 输出 24 个脉冲依次读出补码数据；
 * 3. 读完 24bit 后继续输出 1~3 个 SCK 脉冲选择下一轮通道/增益；
 * 4. SCK 长时间保持高电平会让 HX711 进入掉电状态，所以初始化和每次读完都保持低电平。
 *
 * 维护要求：
 * - 本文件只处理 GPIO 时序、去皮和标定数学关系，不输出串口日志；
 * - 用户可见文本、命令格式和错误提示必须维护在 `weight_service.c`；
 * - 修改增益脉冲或默认接线时，必须同步更新 `HX711_Handle_t` 成员注释和默认宏说明。
 */
